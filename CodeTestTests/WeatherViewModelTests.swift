...
fileprivate struct WeatherServiceMock: WeatherService {
    ...
    func fetchLocations(completion: @escaping (Result<[WeatherLocation], Error>) -> ()) {
        guard returnsFailure
            ...
            return completion(.success([WeatherLocation(id: UUID().uuidString, name: "Test location 1", status: .barelySunny, temperature: 10),
                                        WeatherLocation(id: UUID().uuidString, name: "Test location 2", status: .cloudy, temperature: 15)]))
        }
